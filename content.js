...
// 立即执行的日志，确认脚本加载
console.log('[Content] ========================================');
console.log('[Content] AI翻译助手 v3.0.0 已加载');
console.log('[Content] 加载时间:', new Date().toISOString());
console.log('[Content] 页面URL:', window.location.href);
console.log('[Content] ========================================');

(function() {
  'use strict';

  console.log('[Content] IIFE 函数开始执行');
  ...
  function init() {
    chrome.storage.sync.get(['translateConfig'], (result) => {
      ...
      console.log('[Content] 翻译助手已加载 - 版本: v3.0.0');
      console.log('[Content] 当前配置:', config);
      console.log('[Content] showOriginal:', config.showOriginal);
  ...
  async function translateText(text, sourceLang) {
    ...
    try {
      ...
      console.log('[Content] 发送翻译请求 - 原文:', text);
      console.log('[Content] 源语言:', sourceLang, '目标语言:', config.targetLanguage);
      ...
      console.log('[Content] 收到完整响应:', response);
      
      if (response.success) {
        console.log('[Content] 收到翻译响应 - 原文:', text);
        console.log('[Content] 收到翻译响应 - 翻译结果:', response.translation);
        console.log('[Content] 翻译结果原始值:', JSON.stringify(response.translation));
        console.log('[Content] showOriginal配置:', config.showOriginal);
        
        const displayText = config.showOriginal
          ? `${text}\n\n${response.translation}`
          : response.translation;
        
        console.log('[Content] 最终显示文本:', displayText);
        console.log('[Content] 显示文本长度:', displayText.length);
        console.log('[Content] 显示文本包含↓:', displayText.includes('↓'));
        console.log('[Content] 显示文本包含"翻译":', displayText.includes('翻译'));
  ...
  function updateTooltipContent(content, isError = false) {
    ...
    console.log('[Content] updateTooltipContent 被调用');
    console.log('[Content] 接收到的 content 参数:', content);
    console.log('[Content] content 类型:', typeof content);
    console.log('[Content] content 长度:', content?.length);
    console.log('[Content] content 原始值:', JSON.stringify(content));
    console.log('[Content] isError:', isError);

    const contentDiv = tooltipElement.querySelector('.tooltip-content');
    if (contentDiv) {
      const htmlContent = escapeHtml(content).replace(/\n/g, '<br>');
      console.log('[Content] 转换后的 HTML 内容:', htmlContent);
      
      contentDiv.innerHTML = htmlContent;
      
      console.log('[Content] 最终 tooltip 的 innerHTML:', contentDiv.innerHTML);
      console.log('[Content] 最终 tooltip 的 textContent:', contentDiv.textContent);
